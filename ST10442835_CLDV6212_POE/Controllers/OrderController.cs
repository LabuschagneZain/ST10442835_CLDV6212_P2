...
namespace ST10442835_CLDV6212_POE.Controllers
{
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> Create(OrderCreateViewModel model)
        {
            ...
            {
                // Add debug logging
                Console.WriteLine($"Creating order - CustomerId: {model.CustomerId}, ProductId: {model.ProductId}, Quantity: {model.Quantity}");

                var customer = await _api.GetCustomerAsync(model.CustomerId);
                var product = await _api.GetProductAsync(model.ProductId);

                // Add more detailed debugging
                Console.WriteLine($"Customer lookup result: {(customer != null ? "Found" : "Not Found")}");
                Console.WriteLine($"Product lookup result: {(product != null ? "Found" : "Not Found")}");

                if (customer is null || product is null)
                {
                    ModelState.AddModelError(string.Empty,
                        $"Invalid customer or product selected. Customer: {customer is null}, Product: {product is null}");
                    await PopulateDropdowns(model);
                    return View(model);
                }
        ...
        public async Task<IActionResult> Edit(Order posted)
        {
            if (!ModelState.IsValid)
            {
                // Re-populate any necessary data if needed
                return View(posted);
            }

            try
            {
                // Only update the status since other fields should be readonly
                await _api.UpdateOrderStatusAsync(posted.RowKey, posted.Status);
