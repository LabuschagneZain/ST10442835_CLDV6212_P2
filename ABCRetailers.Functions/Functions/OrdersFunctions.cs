...
namespace ABCRetailers.Functions.Functions
{
    public class OrdersFunctions
    {
            ...
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "orders")] HttpRequestData req)
        {
            ...
            {
                return await HttpJson.Bad(req, $"Invalid ProductId: {input.ProductId}");
            }
            ...
            {
                await foreach (var c in customers.QueryAsync<CustomerEntity>(x => x.RowKey == input.CustomerId))
                {
                    customer = c;
                    break;
                }
                ...
                if (customer == null)
                    return await HttpJson.Bad(req, $"Invalid CustomerId: {input.CustomerId}. Ensure the customer exists in Azure Table Storage with PartitionKey='Customer' and RowKey='{input.CustomerId}'.");
            }
